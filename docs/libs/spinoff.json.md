...
- **Clone**
  - ```rust
    fn clone(self: &Self) -> SpinnerFrames { /* ... */ }
    ```
...
#### Struct `CircleQuarters`

**Attributes:**

- `#[<cfg>(feature = "circle_quarters")]`
...
- ```rust
  pub fn update_text<T>(self: &mut Self, msg: T)
where
    T: Into<Cow<''static, str>> { /* ... */ }
  ```
...
- ```rust
  pub fn update_after_time<T>(self: &mut Self, updated_msg: T, duration: Duration)
where
    T: Into<Cow<''static, str>> { /* ... */ }
  ```
