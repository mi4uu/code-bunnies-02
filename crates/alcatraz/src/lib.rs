...
impl Project {
    ...
    pub fn command<I, S>(&self, args: I) -> anyhow::Result<Output>
    where
        I: IntoIterator<Item = S>,
        S: AsRef<OsStr>,
    {
        let tmpdir = Path::new(&std::env::var("SANDBOX_TARGET_DIR")?)
            .join("debug")
        ...
        let tmpdir = Path::new(&std::env::var("SANDBOX_TARGET_DIR")?)
            ...
            .join(std::env::var("SANDBOX_PKG_NAME")?);
        #[cfg(feature = "release")]
        let tmpdir = Path::new(&std::env::var("CARGO_MANIFEST_DIR")?)
            .join("target")
            .join("release")
            .join(env!("CARGO_PKG_NAME"));

        let r = Command::new(tmpdir)
...
pub trait WithStdout {
    ...
    /// ## Example
    /// ```no_run
    /// # use crate::alcatraz::WithStdout;
    /// # use std::error::Error;
    /// # use alcatraz::project;
...
///
/// ```no_run
/// # use alcatraz::{project, fuzz, WithStdout};
/// # use std::error::Error;
/// # fn main() -> Result<(), Box<dyn Error>> {
/// let proj = project()?;
...
///
/// ```no_run
/// # use alcatraz::{project, fuzz_seed, WithStdout};
/// # use std::error::Error;
/// # fn main() -> Result<(), Box<dyn Error>> {
/// let proj = project()?;
